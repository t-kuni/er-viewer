  ...
  handleMouseDown(event) {
    ...
    switch (currentState.interactionMode) {
      ...
      default:
        ...
        const resizeHandle =
          event.target && event.target.classList && event.target.classList.contains('resize-handle')
            ? event.target
            : null;
        if (resizeHandle) {
          ...
          if (textElement) {
            ...
            const rectangleElement =
              event.target && event.target.closest
                ? event.target.closest('.annotation-rectangle') || event.target.closest('.annotation-rectangle-group')
                : null;
            if (rectangleElement) {
