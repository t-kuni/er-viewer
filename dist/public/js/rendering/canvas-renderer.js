...
export class CanvasRenderer {
    ...
    renderER(erData, layoutData, layerManager = null) {
        ...
        if (layerManager && layerManager.getLayerOrder()) {
            ...
            this.renderByLayerOrder(erData || { entities: [], relationships: [] }, layoutData || { entities: {}, rectangles: [], texts: [] }, layerManager);
        }
        else {
            // Fallback to default order
            if (erData && erData.relationships && layoutData) {
                // Render relationships first (behind entities)
                const positions = convertLayoutsToPositions(layoutData.entities);
            ...
            if (erData && erData.entities && layoutData) {
                ...
                const entityLayouts = layoutData.entities;
     ...
     */
    renderEntitiesInDynamicLayer(entities, entityLayouts) {
        const dynamicLayer = document.getElementById('dynamic-layer');
        if (!dynamicLayer) {
            return;
        }
    ...
    createEntityElement(entity, position) {
        ...
        const entityHeight = this.config.entity.headerHeight + columnCount * this.config.entity.columnHeight + this.config.entity.padding;
