...
import { createERData, createUserEntity, createUserPostERData, createNetworkResponse, createDDLResponse } from './test-data-factory';
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    ...
    describe('注釈追加', () => {
        test('矩形注釈を追加できる', () => {
            ...
            // DOM要素の追加を検証
            expect(appendChildSpy).toHaveBeenCalledWith(expect.anything(), // annotation-layer
        ...
        test('テキスト注釈追加をキャンセルできる', () => {
            ...
            // text要素が作成されないことを確認
            const textElementCalls = createElementSpy.mock.calls.filter(call => call[0] === 'text');
    ...
    describe('エンティティホバー時のハイライト', () => {
        test.skip('エンティティにホバーすると関連エンティティとリレーションがハイライトされる', async () => {
            ...
            const mousemoveHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === canvasElement && call[1] === 'mousemove');
        ...
        test.skip('リレーションにホバーすると両端のエンティティとカラムがハイライトされる', async () => {
            ...
            hasClassSpy.mockImplementation((element, className) => {
                return element === mockRelationship && className === 'relationship';
            });
            // getAttribute()の実装
            getAttributeSpy.mockImplementation((element, attr) => {
    ...
    describe('マウスホイールによる拡大縮小', () => {
        ...
        test('マウスホイールを上に回すと拡大される', async () => {
            ...
            // カンバスのイベントリスナーを探して呼び出し
            const canvasElement = infrastructure.dom.getElementById('er-canvas');
            const wheelHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === canvasElement && call[1] === 'wheel');
    ...
    describe('スペースキーとドラッグによるスクロール', () => {
        ...
        test('スペースキーを押しながらドラッグするとスクロールできる', async () => {
            ...
            const keydownHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === documentElement && call[1] === 'keydown');
            ...
            const mousedownHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === canvasElement && call[1] === 'mousedown');
            ...
            const mousemoveHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === documentElement && call[1] === 'mousemove');
            if (mousemoveHandler && mousemoveHandler[2]) {
            ...
            const mouseupHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === documentElement && call[1] === 'mouseup');
            ...
            const keyupHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === documentElement && call[1] === 'keyup');
        ...
        test('マウスホイール押し込みながらドラッグでもスクロールできる', async () => {
            ...
            const mousemoveHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === infrastructure.dom.getDocumentElement() && call[1] === 'mousemove');
            if (mousemoveHandler && mousemoveHandler[2]) {
                mousemoveHandler[2](mockMouseMoveEvent);
            }
            // Assert - パン位置が変更されている
...
//# sourceMappingURL=user-interaction.test.js.map