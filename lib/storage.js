...
class StorageManager {
  ...
  async saveERData(erData) {
    ...
    // Remove position information from entities before saving
    const cleanedData = {
      ...erData,
      entities: erData.entities.map(entity => {
        const { position, ...entityWithoutPosition } = entity;
        return entityWithoutPosition;
      })
    };
    await fs.writeFile(this.erDataFile, JSON.stringify(cleanedData, null, 2));
  }
  ...
  async mergeERDataWithLayout(erData, layoutData) {
    ...
    // Position information is now managed only in layoutData
    // No need to add position to entities
