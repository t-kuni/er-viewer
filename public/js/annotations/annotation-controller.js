...
export class AnnotationController {
    constructor(stateManager, coordinateTransform) {
    ...
    editRectangleProperties(rectangleElement) {
        const index = parseInt(rectangleElement.getAttribute('data-index'));
        ...
        document.getElementById('save-rect-props').addEventListener('click', () => {
            ...
            const opacity = document.getElementById('rect-fill-opacity').value / 100;
    ...
    /**
     * Delete annotation
     * @param {Element} annotationElement - Annotation DOM element
     */
    deleteAnnotation(annotationElement) {
        if (!annotationElement) return;
        
        const type = annotationElement.getAttribute('data-type');
        const index = parseInt(annotationElement.getAttribute('data-index'));
        const currentState = this.stateManager.getState();
        const newLayoutData = { ...currentState.layoutData };
     ...
     */
    rgbaToHex(rgba) {
        if (!rgba || !rgba.startsWith('rgba')) return '#3498db';
    ...
    getOpacityPercent(rgba) {
        if (!rgba || !rgba.startsWith('rgba')) return 10;
    ...
    hexToRgba(hex, alpha) {
        ...
        if (!result) return 'rgba(52, 152, 219, 0.1)';
