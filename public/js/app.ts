...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    ClientLogger.error(
      'Failed to initialize ER Viewer application',
...
function setupGlobalEventListeners(): void {
  ...
  const resizeHandle = sidebar?.querySelector('.sidebar-resize-handle') as HTMLElement | null;
...
function setupHelpPanel(): void {
  ...
  // Add event listeners
  helpToggle.addEventListener('click', (e: Event) => {
    e.stopPropagation();
    toggleHelpPanel();
  });
...
async function reverseEngineer(): Promise<void> {
