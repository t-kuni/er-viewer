...
export class ERViewerApplication {
     ...
     */
    initializeWhenReady() {
        if (this.infra.dom.getDocumentElement().readyState === 'loading') {
    ...
    initialize() {
        ...
        this.infra.browser.log('ER Viewer application initialized successfully');
    }
    ...
    initializeCanvas() {
        ...
        this.infra.dom.setAttribute(this.state.canvas, 'width', width);
        this.infra.dom.setAttribute(this.state.canvas, 'height', height);
    ...
    async loadERData() {
        try {
            this.infra.browser.log('Loading ER data...');
            ...
            this.infra.browser.log('ER data loaded successfully:', erData);
        } catch (error) {
            this.infra.browser.error('Error loading ER data:', error);
    ...
    subscribeToProperty(key, callback) {
        ...
        this.propertySubscribers.get(key).add(callback);
        return () => {
            if (this.propertySubscribers.has(key)) {
    ...
    notifySubscribers(oldState, newState) {
        ...
        this.subscribers.forEach(callback => {
            try {
                ...
                this.infra.browser.error('Error in state subscriber:', error);
            }
        });
            ...
            if (oldState[key] !== newState[key]) {
                callbacks.forEach(callback => {
                    try {
                        ...
                        this.infra.browser.error(`Error in property subscriber for ${key}:`, error);
                    }
                });
            }
        });
    ...
    shouldRerender(updates) {
        const renderKeys = ['erData', 'layoutData', 'viewport', 'highlightedEntities', 'highlightedRelationships'];
     ...
     */
    renderEntities() {
        if (!this.state.erData || !this.state.erData.entities) return;
    ...
    createEntityElement(entity, position) {
        ...
        this.infra.dom.setAttribute(rect, 'width', width);
        this.infra.dom.setAttribute(rect, 'height', height);
        ...
        this.infra.dom.setAttribute(headerRect, 'width', width);
        this.infra.dom.setAttribute(headerRect, 'height', headerHeight);
        ...
        this.infra.dom.setAttribute(title, 'x', width / 2);
        this.infra.dom.setAttribute(title, 'y', headerHeight / 2 + 5);
        ...
        entity.columns.forEach((column, index) => {
            ...
            this.infra.dom.setAttribute(columnText, 'x', padding);
            this.infra.dom.setAttribute(columnText, 'y', y);
    ...
    getEntityPosition(entityName) {
        ...
        if (this.state.layoutData.entities[entityName]) {
            return this.state.layoutData.entities[entityName];
        }
     ...
     */
    renderRelationships() {
        if (!this.state.erData || !this.state.erData.relationships) return;
        
        const dynamicLayer = this.infra.dom.getElementById('dynamic-layer');
        if (!dynamicLayer) return;
        
        // Remove existing relationship groups first
        // dynamicLayer.children は HTMLCollection なので Array.from() で配列に変換
        const existingGroups = Array.from(dynamicLayer.children).filter(child => 
            child.getAttribute && child.getAttribute('class') === 'relationships'
        );
        ...
        const firstEntity = Array.from(dynamicLayer.children).find(child => 
            child.getAttribute && child.getAttribute('class') && 
            child.getAttribute('class').includes('entity')
        );
    ...
    createRelationshipPath(relationship) {
        ...
        this.infra.dom.setAttribute(path, 'data-from-table', relationship.from.table);
        this.infra.dom.setAttribute(path, 'data-to-table', relationship.to.table);
    ...
    createRectangleAnnotation(rect, index) {
        ...
        this.infra.dom.setAttribute(rectElement, 'data-rect-index', index);
        this.infra.dom.setAttribute(rectElement, 'x', rect.x);
        this.infra.dom.setAttribute(rectElement, 'y', rect.y);
        this.infra.dom.setAttribute(rectElement, 'width', rect.width);
        this.infra.dom.setAttribute(rectElement, 'height', rect.height);
        this.infra.dom.setAttribute(rectElement, 'fill', rect.fill || '#e3f2fd');
        this.infra.dom.setAttribute(rectElement, 'stroke', rect.stroke || '#1976d2');
        this.infra.dom.setAttribute(rectElement, 'stroke-width', rect.strokeWidth || 2);
        return rectElement;
    }
    ...
    createTextAnnotation(text, index) {
        ...
        this.infra.dom.setAttribute(textElement, 'data-text-index', index);
        this.infra.dom.setAttribute(textElement, 'x', text.x);
        this.infra.dom.setAttribute(textElement, 'y', text.y);
        this.infra.dom.setAttribute(textElement, 'fill', text.color || '#2c3e50');
        this.infra.dom.setAttribute(textElement, 'font-size', text.size || 14);
    ...
    handleCanvasMouseDown(event) {
        ...
        if (this.infra.dom.hasClass(target, 'annotation-rectangle') || 
            this.infra.dom.hasClass(target, 'annotation-text')) {
            event.preventDefault();
     ...
     */
    updateDrag(event) {
        if (!this.state.dragState || this.state.dragState.type !== 'entity') return;
        ...
        this.infra.dom.setAttribute(
            this.state.dragState.element,
            'transform',
            `translate(${newX}, ${newY})`
        );
     ...
     */
    updatePan(event) {
        if (!this.state.dragState || this.state.dragState.type !== 'pan') return;
        ...
        if (this.state.interactionMode === 'dragging' && this.state.dragState) {
            ...
            if (this.state.dragState.type === 'entity') {
                ...
                newLayoutData.entities[this.state.dragState.tableName] = {
                    x: this.state.entityBounds.get(this.state.dragState.tableName).x,
                    y: this.state.entityBounds.get(this.state.dragState.tableName).y
                };
    ...
    async showTableDetails(tableName) {
        try {
            this.infra.browser.log('showTableDetails called with tableName:', tableName);
            const response = await this.infra.network.fetch(`/api/table/${tableName}/ddl`);
            
            if (response.ok) {
                const data = await response.json();
                this.infra.browser.log('DDL data received:', data);
                this.showSidebar(tableName, data.ddl);
            } else {
                this.infra.browser.error('Failed to fetch DDL:', response.status, response.statusText);
            }
        } catch (error) {
            this.infra.browser.error('Error loading table details:', error);
        }
    }
    ...
    showSidebar(tableName, ddl) {
        if (!this.state.sidebar || !this.state.sidebarContent) return;
     ...
     * Show context menu
     */
    showContextMenu(screenX, screenY, svgPoint, target) {
    ...
    hideContextMenu() {
        if (this.state.contextMenu) {
            this.infra.dom.removeElement(this.state.contextMenu);
    ...
    addRectangleAtPosition(x, y) {
        ...
        this.infra.browser.log('Rectangle added at:', x, y);
    }
    ...
    addTextAtPosition(x, y) {
        const text = this.infra.browser.prompt('テキストを入力してください:');
        if (!text) return;
        ...
        if (reverseBtn) {
            this.infra.dom.addEventListener(reverseBtn, 'click', async () => {
                try {
                    ...
                    this.infra.browser.error('Reverse engineering failed', error);
                }
            });
        }
        ...
        if (saveBtn) {
            this.infra.dom.addEventListener(saveBtn, 'click', async () => {
                try {
                    ...
                    this.infra.browser.error('Save layout failed', error);
                }
            });
        }
    ...
    async reverseEngineer() {
        this.infra.browser.log('Starting reverse engineering...');
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.infra.browser.error(`Reverse engineering failed: ${response.status} ${response.statusText}`, errorText);
                this.showError('リバースエンジニアリングに失敗しました', `${response.status}: ${errorText}`);
            }
        } catch (error) {
            this.infra.browser.error('Error during reverse engineering:', error);
    ...
    async saveLayout() {
        try {
            const response = await this.infra.network.postJSON('/api/layout', this.state.layoutData);
            this.showSuccess('レイアウトが保存されました');
        } catch (error) {
            this.infra.browser.error('Error saving layout:', error);
            this.showError('レイアウトの保存に失敗しました', error.message);
        }
    }
    ...
    setupHelpPanelEvents() {
        ...
        if (!helpPanel || !helpToggle || !helpContent || !helpHeader) {
            this.infra.browser.warn('Help panel elements not found');
            return;
        }
    ...
    setupLayerOrderChangeEvents() {
        this.infra.dom.addEventListener(this.infra.dom.getDocumentElement(), 'layerOrderChanged', (e) => {
            this.infra.browser.log('Layer order changed:', e.detail);
            if (e.detail && e.detail.layers) {
                const newLayoutData = { ...this.state.layoutData };
    ...
    setupResizeHandler() {
        ...
        this.infra.browser.addWindowEventListener('resize', () => {
            this.resizeCanvas();
        });
    }
    ...
    selectAnnotation(element) {
        this.setState({ selectedAnnotation: element });
