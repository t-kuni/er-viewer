...
- プロジェクトルートディレクトリ（`rootDir`）: 開発時と本番時で実行ファイルの位置が異なるため、`server.ts`で計算した値を注入する
...
- **依存性注入**: Usecaseに必要な依存性（副作用）を注入してインスタンス化
  - プロジェクトルート（`rootDir`）の計算: 開発時は`__dirname`、本番時は`path.join(__dirname, '..')`を使用
- **HTTPレスポンス処理**: Usecaseの結果をHTTPレスポンスに変換
- **エラーハンドリング**: エラーを適切なHTTPステータスコードに変換
- **静的ファイル配信**: フロントエンドのビルド済みファイル（`public/dist/`）を配信
