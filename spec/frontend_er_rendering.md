...
* `ERDiagramViewModel`の連想配列形式により、IDから要素を直接取得可能（O(1)）
* エンティティホバー時: 全エッジから該当エンティティに接続されているものを検索、その反対側のエンティティを連想配列から直接取得
* エッジホバー時: エッジから`source/target`を取得し、両端のエンティティを連想配列から直接取得、対応カラムを検索
* カラムホバー時: 全エッジから該当カラムが関係するものを検索、反対側のエンティティとカラムを取得
...
* 連想配列形式により、IDから要素を直接取得可能（O(1)）
* エッジの検索は`Object.values(edges).filter(...)`で実施（MVPフェーズではパフォーマンスは許容範囲）
* React.memoやuseMemoを活用してレンダリングを最適化
* カラムホバーはエンティティホバーやエッジホバーより複雑なため、段階的に実装することを推奨
