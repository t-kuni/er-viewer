...
ER図キャンバス全体を自由にスクロール（パン）するため、スペースキーを押しながらのドラッグ操作を提供する。
この操作中は、エンティティやリレーションへのインタラクション（ホバー、ドラッグ）を無効化し、純粋なビューポート移動のみを実現する。
...
**スペースキー押下の効果**:
* エンティティドラッグの無効化により、エンティティ上でも確実にパンスクロールできる
* 通常時も背景ドラッグでパンは可能だが、スペースキーを使うとエンティティを避けてパンする必要がなくなる

**無効化される機能**:
* エンティティのドラッグ: スペース押下中はエンティティをドラッグできない
* ホバーインタラクション: スペース押下中はホバーイベントを無視する
  - エンティティ、エッジ、カラムへのホバーでハイライト表示されない
  - 既にハイライト表示されている場合はクリアされる
* 矩形・テキストのドラッグ: スペース押下中は矩形・テキストもドラッグできない
...
* `panOnDrag`と`nodesDraggable`の切り替えはスペースキー押下状態の変化時に即座に反映される
