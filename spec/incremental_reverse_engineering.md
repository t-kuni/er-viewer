...
**既存エンティティ（テーブル名が一致）**
- 既存のid、x座標、y座標を維持
- width、heightは0にリセット（カラム数が変わる可能性があるため、レンダリング後に再計測）
...
エンティティごとに、データベースから取得したカラム情報で全件置き換え。
...
データベースから取得したリレーションシップ情報で全件置き換え。
