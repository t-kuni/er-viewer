 ...
 */
import { ERViewerApplication } from '../public/js/er-viewer-application.js';
import { InfrastructureMock } from '../public/js/infrastructure/mocks/infrastructure-mock.js';
...
describe('ERViewerApplication E2E Tests', () => {
    ...
    describe('エンティティの表示', () => {
        ...
        test('エンティティが正しく描画される', () => {
            ...
            // エンティティ要素が作成されることを確認
            const entityElements = dynamicLayer.children.filter(child => 
                child.getAttribute && child.getAttribute('class') === 'entity draggable'
            );
        ...
        test('リレーションシップが正しく描画される - dynamicLayer.children.filter エラー修正', () => {
            ...
            const relationshipGroups = childrenArray.filter(child => 
            ...
            const pathElements = Array.from(relationshipGroup.children).filter(child =>
                child.tagName === 'path' && child.getAttribute('class') === 'relationship'
            );
        ...
        test('リレーションシップレンダリングの詳細検証', () => {
            ...
            expect(relationship.from.table).toBe('posts');
            expect(relationship.to.table).toBe('users');
            ...
            const relationshipGroups = dynamicLayer.children.filter(child => 
                child.getAttribute && child.getAttribute('class') === 'relationships'
            );
            
            if (relationshipGroups.length === 0) {
                console.log('No relationship groups found');
                console.log('Available children classes:', 
                    dynamicLayer.children.map(child => child.getAttribute('class')));
            }
        ...
        test('リレーションシップパスの座標が正しく計算される', () => {
            ...
            const relationshipGroup = relationshipGroups[0];
            const pathElements = relationshipGroup.children.filter(child =>
                child.tagName === 'path' && child.getAttribute('class') === 'relationship'
            );
    ...
    describe('エンティティのドラッグ操作', () => {
        ...
        test('エンティティをドラッグできる', () => {
            // usersエンティティの要素を取得
            const dynamicLayer = infrastructure.dom.getElementById('dynamic-layer');
            const userEntity = dynamicLayer.children.find(child => 
                child.getAttribute && child.getAttribute('data-table-name') === 'users'
            );
    ...
    describe('UI操作', () => {
        ...
        test('ローディング表示が正常に動作する', () => {
            ...
            expect(loadingElement.innerHTML).toBe('テスト中...');
    ...
    describe('エラーハンドリング', () => {
        ...
        test('無効なテーブル名でのDDL取得エラーが処理される', async () => {
            infrastructure.network.setMockResponse('/api/table/invalid/ddl', {
                status: 404,
