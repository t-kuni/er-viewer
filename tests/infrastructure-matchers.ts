 ...
 * DOM要素の存在を検証するマッチャー
 */
export const toHaveElement = (infrastructure: InfrastructureMock, elementId: string) => {
 ...
 * DOM要素の属性を検証するマッチャー
 */
export const toHaveAttribute = (element: MockElement, attributeName: string, expectedValue: string) => {
 ...
 * DOM要素のクラスを検証するマッチャー
 */
export const toHaveClass = (element: MockElement, className: string) => {
 ...
 */
export const toHaveTextContent = (element: MockElement, expectedText: string) => {
 ...
 */
export const toHaveMadeRequest = (infrastructure: InfrastructureMock, url: string, method?: string) => {
 ...
 * リクエストボディを検証するマッチャー
 */
export const toHaveRequestedWithBody = (infrastructure: InfrastructureMock, url: string, expectedBody: any) => {
  const history = infrastructure.getInteractionHistory();
  ...
  const actualBody = typeof request.body === 'string' ? JSON.parse(request.body) : request.body;
 ...
 * Storage操作を検証するマッチャー
 */
export const toHaveStoredItem = (infrastructure: InfrastructureMock, key: string, value?: any) => {
  ...
  if (value === undefined) {
    return {
      pass: hasKey,
      message: () =>
        hasKey ? `Expected storage not to contain key "${key}"` : `Expected storage to contain key "${key}"`,
    };
  }
...
export const toHaveSetAttribute = (
  ...
  // 要素を取得
  const targetElement =
    typeof element === 'string' ? (infrastructure.dom.getElementById(element) as unknown as MockElement) : element;
 ...
 * エラーログを検証するマッチャー
 */
export const toHaveLoggedError = (infrastructure: InfrastructureMock, errorMessage: string) => {
  const history = infrastructure.getInteractionHistory();
  const errors = history.errors;

  const matchingError = errors.find((error) =>
    error.args.some((arg) => typeof arg === 'string' && arg.includes(errorMessage)),
  );
