...
export default class MockCoverageReporter implements Reporter {
   ...
   * HTMLレポートを生成
   */
  private generateHtmlReport(tracker: MockCoverageTracker): string {
    const summary = tracker.generateSummary();
    const coverageData = (tracker as any).coverageData; // プライベートプロパティへのアクセス
...
<body>
  ...
  private getAllMethodsForMock(mockName: string, details: any, coverageData: any): any[] {
    const covered = coverageData[mockName] || {};
    const methods: any[] = [];

    // カバーされたメソッド
    Object.entries(covered).forEach(([methodName, data]: [string, any]) => {
      methods.push({
        name: methodName,
        covered: true,
    ...
    // カバーされていないメソッド
    details.uncoveredMethods.forEach((methodName: string) => {
      methods.push({
        name: methodName,
        covered: false,
        callCount: 0,
