...
describe('レイヤー一覧のドラッグ&ドロップ', () => {
  ...
  test('レイヤーアイテムをドラッグ&ドロップで順番を入れ替えられる', () => {
    ...
    expect((updatedLayerItems[1] as unknown as MockElement).innerHTML).toContain('矩形No2');
    expect((updatedLayerItems[2] as unknown as MockElement).innerHTML).toContain('矩形No1');
  });
