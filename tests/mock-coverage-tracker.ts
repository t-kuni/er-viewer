...
interface MockCoverageData {
  [mockName: string]: {
    [methodName: string]: {
      callCount: number;
      calls: MockCallInfo[];
      firstCalledIn: string;
      lastCalledIn: string;
    };
  };
}
...
export class MockCoverageTracker {
  private static instance: MockCoverageTracker;
  private coverageData: MockCoverageData = {};
  private currentTest: { name: string; file: string } | null = null;
  private allMethods: Map<string, Set<string>> = new Map();
  ...
  generateSummary(): {
    ...
    const allUncoveredMethods: string[] = [];
    let totalMethods = 0;
  ...
  generateDetailedReport(): string {
    ...
    const allCalls: { mock: string; method: string; count: number }[] = [];
  ...
  static clearInstance(): void {
    if (MockCoverageTracker.instance) {
      ...
      MockCoverageTracker.instance = undefined as any;
    }
  }
}
