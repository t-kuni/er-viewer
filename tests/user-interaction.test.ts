...
describe('ユーザーインタラクション', () => {
  ...
  describe('エンティティ選択', () => {
    ...
    test('エンティティ要素をクリックするとhandleCanvasClickが正しく動作する', async () => {
      ...
      const closestSpy = jest
        .spyOn(infrastructure.dom, 'closest')
        .mockReturnValue(mockEntityElement as unknown as Element);

      // getAttribute メソッドのモック
      const getAttributeSpy = jest.spyOn(infrastructure.dom, 'getAttribute').mockImplementation((element, attrName) => {
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      ...
      const querySelectorSpy = jest
        .spyOn(infrastructure.dom, 'querySelector')
  ...
  describe('注釈追加', () => {
    ...
    test('テキスト注釈追加をキャンセルできる', () => {
      ...
      expect(setStylesSpy).not.toHaveBeenCalledWith(expect.objectContaining({ tagName: 'text' }), expect.any(Object));
  ...
  describe('マウスホイールによる拡大縮小', () => {
    ...
    test('マウスホイールを上に回すと拡大される', async () => {
      ...
      expect(setAttributeSpy).toHaveBeenCalledWith(expect.anything(), 'transform', expect.stringContaining('scale(1)'));
  ...
  describe('テキスト描画機能', () => {
    ...
    test('テキスト描画モードでクリックするとテキストが追加される', async () => {
      ...
      // カーソルが変更されたことを確認
      expect(setStylesSpy).toHaveBeenCalledWith(expect.anything(), { cursor: 'text' });
      ...
      expect(setStylesSpy).toHaveBeenCalledWith(expect.anything(), { cursor: 'default' });
